...
router.post("/create-contact", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const recipientUser = await User.findOne({_id : contacts[0]})
    ...
    // console.log("this is the recipient user", senderDirectMessage)
    // console.log("this is the current user", recipientDirectMessage)
